...
import { convertToUint8Array, convertToArrayBuffer, sha1BytesSync, bytesCmp, bytesToHex, bytesFromArrayBuffer, bytesToArrayBuffer, longToBytes, uintToInt, rshift32 } from '../../bin';
...
var storeIntString = writer => (value, field) => {
  switch (typeof value) {
    case 'string':
      return writeBytes(writer, value, `${field}:string`);
    case 'number':
...
export class NetworkerThread {
  ...
  parseResponse(responseBuffer) {
    ...
    return _asyncToGenerator(function* () {
      ...
      var messageID = readLong(deserializer.typeBuffer, 'message_id');
...
var _initialiseProps = function () {
  ...
  function* _ref15(event) {
    ...
    smartTimeout.cancel(_this4.checkConnectionPromise);

    var serializer = _this4.Serialization({ mtproto: true });
    var pingID = getRandomId();

    serializer.storeMethod('ping', { ping_id: pingID });

    var pingMessage = new NetMessage(_this4.generateSeqNo(true), serializer.getBytes());
    ...
    _this4.checkConnectionPromise = smartTimeout(_this4.checkConnection, delay);
  ...
  this.performSheduledRequest = () => {
    ...
    if (message.isAPI && !message.longPoll) {
      ...
      messages.push(new NetMessage(this.generateSeqNo(), serializer.getBytes()));
    ...
    if (messages.length > 1) {
      ...
      message = new NetContainer(this.generateSeqNo(true), container.getBytes(true), innerMessages);
  ...
  function* _ref20(message, options = {}) {
    ...
    writeIntBytes(dataBox, _this4.serverSalt, 64, 'salt');
    writeIntBytes(dataBox, _this4.sessionID, 64, 'session_id');
    ...
    writeIntBytes(requestBox, _this4.authKeyID, 64, 'auth_key_id');
...
export var getDeserializeOpts = msgGetter => ({
  mtproto: true,
  override: {
